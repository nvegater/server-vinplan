...
export class ReservationResolver {
  ...
  async allReservations(
    ...
    const paginatedReservations = await getConnection().query(
      SQL_QUERY_SELECT_RESERVATIONS_WITH_USER_AND_SERVICE,
      replacements
    );
    ...
    if (paginatedReservations !== undefined) {
      return {
        ...
        moreReservationsAvailable:
          paginatedReservations.length === realLimit + 1, // DB has more posts than requested
      };
    } else {
