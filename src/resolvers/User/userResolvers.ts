...
import {
  ...
  UserResponse,
  WineryResponse,
  SendUserValidationResponse,
} from "./userResolversOutputs";
import {
...
export class UserResolver {
  ...
  async me(@Ctx() { req }: ApolloRedisContext): Promise<UserResponse> {
    // @ts-ignore
  ...
  async register(
    ...
    const registerResult: UserResponse = await registerUser(
      registerInputs,
      redis
    );
  ...
  async changePassword(
    ...
    const inputErrors: FieldError[] =
      validateInputsChangePassword(changePasswordInputs);
    if (inputErrors.length > 0) {
      return { errors: inputErrors };
    }
    ...
    if (!userId) {
      ...
      if (!user) {
        return {
          errors: inputErrors.concat(userResolversErrors.tokenUserError),
        };
      } else {
        await User.update(
          {
  ...
  @Mutation(() => Boolean)
  async logout(@Ctx() { req, res }: ApolloRedisContext) {
