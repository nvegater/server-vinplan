...
import {
  arrayAreEquals,
  getDifferentsElements,
} from "../../utils/arrayUtilities";

const updateWinery = async (
  updateWineryInputs: UpdateWineryInputs
  ...
  try {
    ...
    if (updateWineryInputs.wineGrapesProduction) {
      arrayPromises.push(
        saveWineryGrapesProduction(
          updateWineryInputs.id,
          updateWineryInputs.wineGrapesProduction
        )
      );
    }
    ...
    if (updateWineryInputs.othersServices) {
      arrayPromises.push(
        saveWineryOtherServices(
          updateWineryInputs.id,
          updateWineryInputs.othersServices
        )
      );
    }
    ...
    if (updateWineryInputs.productionType) {
      arrayPromises.push(
        saveWineryProductionType(
          updateWineryInputs.id,
          updateWineryInputs.productionType
        )
      );
    }
    ...
    if (updateWineryInputs.wineType) {
      arrayPromises.push(
        saveWineryWineType(updateWineryInputs.id, updateWineryInputs.wineType)
      );
    }

    if (updateWineryInputs.supportedLanguages) {
      arrayPromises.push(
        saveWinerySupportedLanguages(
          updateWineryInputs.id,
          updateWineryInputs.supportedLanguages
        )
      );
    }
    ...
    if (updateWineryInputs.amenities) {
      arrayPromises.push(
        saveWineryAmenity(updateWineryInputs.id, updateWineryInputs.amenities)
      );
    }
...
const saveWineryGrapesProduction = async (
  id: number,
  wineGrapesProduction: Grape[]
) => {
  const wineGrapesF = await WineryGrapesServicesResponse.getWineGrapesById(id);
  const wineGrapesFound = wineGrapesF.map(
    (grapes) => grapes.wineGrapesProduction
  );
  if (!arrayAreEquals(wineGrapesFound, wineGrapesProduction)) {
    //son diferentes
    //se van a insertar
    getDifferentsElements(wineGrapesProduction, wineGrapesFound).forEach(
      async (grape) => {
        await WineryGrapesServicesResponse.insertGrapesToWinery(id, grape);
      }
    );
    //se van a eliminar
    getDifferentsElements(wineGrapesFound, wineGrapesProduction).forEach(
      async (grape) =>
        await WineryGrapesServicesResponse.deleteGrapesToWinery(id, grape)
    );
  }
};

const saveWineryOtherServices = async (
  id: number,
  othersServices: OtherServices[]
) => {
  const servicesF =
    await WineryOtherServicesResponse.getWineryOtherServicesById(id);
  const servicesFound = servicesF.map((oServices) => oServices.service);

  if (!arrayAreEquals(servicesFound, othersServices)) {
    //son diferentes
    //se van a insertar
    getDifferentsElements(othersServices, servicesFound).forEach(
      async (service) => {
        await WineryOtherServicesResponse.insertOtherServiceToWinery(
          id,
          service
        );
      }
    ...
    //se van a eliminar
    getDifferentsElements(servicesFound, othersServices).forEach(
      async (service) =>
        await WineryOtherServicesResponse.deleteOtherServiceToWinery(
          id,
          service
        )
    );
...
const saveWineryProductionType = async (
  id: number,
  productionType: ProductionType[]
) => {
  const productionTypesF =
    await WineProductionTypeServices.getProductionTypeByWineryId(id);
  const productionTypesFound = productionTypesF.map(
    (prodType) => prodType.productionType
  );

  if (!arrayAreEquals(productionTypesFound, productionType)) {
    //son diferentes
    //se van a insertar
    getDifferentsElements(productionType, productionTypesFound).forEach(
      async (productionT) => {
        await WineProductionTypeServices.insertProductionTypeToWinery(
          id,
          productionT
        );
      }
    ...
    //se van a eliminar
    getDifferentsElements(productionTypesFound, productionType).forEach(
      async (productionT) =>
        await WineProductionTypeServices.deleteProductionTypeToWinery(
          id,
          productionT
        )
    );
...
const saveWineryWineType = async (id: number, wineType: TypeWine[]) => {
  ...
  if (!arrayAreEquals(wineTypesFound, wineType)) {
    //son diferentes
    //se van a insertar
    getDifferentsElements(wineType, wineTypesFound).forEach(async (wineT) => {
      await WineTypeServices.insertWineTypeToWinery(id, wineT);
    });
    //se van a eliminar
    getDifferentsElements(wineTypesFound, wineType).forEach(
      async (wineT) => await WineTypeServices.deleteWineTypeToWinery(id, wineT)
    );
  }
};

const saveWinerySupportedLanguages = async (
  id: number,
  supportedLanguages: SupportedLanguage[]
) => {
  const wineSupportedLanguajesF =
    await wineryLanguajeServices.getWineryLanguageByWineryId(id);
  const wineSupportedLanguajesFound = wineSupportedLanguajesF.map(
    (supportLanguaje) => supportLanguaje.supportedLanguage
  );
  ...
  if (!arrayAreEquals(wineSupportedLanguajesFound, supportedLanguages)) {
    ...
    getDifferentsElements(
      supportedLanguages,
      wineSupportedLanguajesFound
    ).forEach(async (supportL) => {
      await wineryLanguajeServices.insertSupportedLanguajeToWinery(
        id,
        supportL
      );
    });
    ...
    getDifferentsElements(
      ...
      async (supportL) =>
        await wineryLanguajeServices.deleteSupportedLanguajeToWinery(
          id,
          supportL
        )
    );
...
const saveWineryAmenity = async (id: number, amenity: Amenity[]) => {
  const wineryAmenitiesF =
    await wineryAmenityServices.getWineryAmenityByWineryId(id);
  const wineryAmenitiesFound = wineryAmenitiesF.map(
    (wineryAmenities) => wineryAmenities.amenity
  );

  if (!arrayAreEquals(wineryAmenitiesFound, amenity)) {
    //son diferentes
    //se van a insertar
    getDifferentsElements(amenity, wineryAmenitiesFound).forEach(
      async (amenity) => {
        await wineryAmenityServices.insertAmenityToWinery(id, amenity);
      }
    );
    //se van a eliminar
    getDifferentsElements(wineryAmenitiesFound, amenity).forEach(
      async (amenity) =>
        await wineryAmenityServices.deleteAmenityToWinery(id, amenity)
    );
  }
};

const saveWineryWithOwnData = async (
  updateWineryInputs: UpdateWineryInputs
) => {
  try {
    const winery: any = await WineryServices.findWineryById(
      updateWineryInputs.id
    );
