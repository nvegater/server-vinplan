...
const showPost = async (limit : number, cursor : string | null, userId: number): Promise<PaginatedPosts> => {
    try{
        ...
        if (cursor) {
            if (userId) {
                resultPosts = await posts.PostsWithCursorUserLogged(limit, userId, cursor);
            } else {
