...
const getServices = async (
  ...
  try {
    ...
    let paginatedServicesDB: Service[] = await services.experiencesWithCursor(
      limit,
      cursor,
      experienceName,
      eventType,
    ...
    for (let i = 0; i < paginatedServicesDB.length; i++) {
      paginatedServicesDB[i].startDateTime = convertDateToUTC(
        paginatedServicesDB[i].startDateTime
      );
      paginatedServicesDB[i].endDateTime = convertDateToUTC(
        paginatedServicesDB[i].endDateTime
      );
      paginatedServicesDB[i].urlImageCover = await getImageCover.experience(
        paginatedServicesDB[i]
      );
      const wineryFound = await wineryServices.findWineryById(
        paginatedServicesDB[i].wineryId
      );
      if (wineryFound) {
    ...
    return {
      ...
      totalExperiences: (
        await services.experiencesWithCursor(
          null,
          null,
          experienceName,
          eventType,
