...
const salesConcentrate = async (paypalTransaccionId : string | null) : Promise<SalesConcentrate> => {
    try {
        ...
        const signedUrl = await s3UploadFile(await buildExcelAsBuffer(excelInfo), filename, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
...
const buildExcelData = (experienceInfo: ServiceReservation[]) => {
    return experienceInfo.map((experience) => {
        return {
            'Fecha de transaction': format(new Date(experience.paymentCreationDateTime), "dd MMMM uuuu"),
