...
const findExperienceById = async (
  experienceId: number
  ...
  try {
    ...
    if (experienceFound != undefined) {
      experienceFound.startDateTime = convertDateToUTC(
        experienceFound.startDateTime
      );
      experienceFound.endDateTime = convertDateToUTC(
        experienceFound.endDateTime
      );
      experienceFound.gallery =
        await ServiceImageGalleryServices.getServiceGalleryById(
          experienceFound?.id
        );
      experienceFound.urlImageCover = await getImageCover.experience(
        experienceFound
      );
      const wineryFound: any = await wineryServices.findWineryById(
        experienceFound.wineryId
      );
      const wineTypesOfWinery: WineType[] | [] =
        await WineTypeServices.getWineTypeByWineryId(wineryFound?.id);
      const prodTypesOfWinery: WineProductionType[] | [] =
        await WineProductionTypeServices.getProductionTypeByWineryId(
          wineryFound?.id
        );
      const languages: WineryLanguage[] | [] =
        await WineryLanguageServices.getWineryLanguageByWineryId(
          wineryFound?.id
        );
      const amenities: WineryAmenity[] | [] =
        await WineryAmenityServices.getWineryAmenityByWineryId(wineryFound?.id);
