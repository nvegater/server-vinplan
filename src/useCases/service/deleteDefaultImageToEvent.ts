...
const deleteDefaultImageToEvent = async (
  eventType: EventType
): Promise<ServiceImageResponse> => {
  try {
    const eventDefaultImage =
      await ServiceImageGalleryServices.findDefaultImageByEventType(eventType);
    if (eventDefaultImage === undefined) {
      ...
      const serviceInserted =
        await ServiceImageGalleryServices.deleteDefaultPictureToEvent(
          eventDefaultImage.id
        );
