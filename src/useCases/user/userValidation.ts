...
const userValidation = async (userId: number): Promise<UserResponse> => {
  ...
  const reservedServicesIds =
    await serviceReservationDataServices.findIdsFromServicesReservedByUserId(
      userId
    );
  const userHasReservations =
    reservedServicesIds[0].reservedServicesIds.length > 0;
  if (userHasReservations) {
    user.reservedServicesIds = reservedServicesIds[0]
      .reservedServicesIds as number[];
    user.reservedServices =
      await serviceReservationDataServices.findUserReservations(userId);
  }
