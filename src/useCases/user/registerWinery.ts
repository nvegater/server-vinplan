...
const registerWinery = async (
  ...
  try {
    ...
    if (userWithUsernameExists) {
      // Level 1
      return {
        errors: inputErrors.concat(userResolversErrors.usernameInUseError),
      };
    } else {
      const userWithEmailExists: User | undefined =
        await userDataServices.findUserByUsername(registerInputs.email);
      if (userWithEmailExists) {
        // Level 2
        return {
          errors: inputErrors.concat(userResolversErrors.emailInUseError),
        };
      } else {
        const wineryWithThatNameExists: Winery | undefined =
          await Winery.findOne({ where: { name: wineryDataInputs.name } });
        if (wineryWithThatNameExists) {
          // Level 3
          return {
            errors: inputErrors.concat(
              userResolversErrors.wineryNameInUseError
            ),
          };
        } else {
          ...
          const newWinery = await userDataServices.createWinery(
            wineryDataInputs,
            creatorId
          );
          ...
          const productionTypes = wineryDataInputs.productionType.map(
            (productionType) => {
          ...
          if (
            wineryDataInputs.supportedLanguages &&
            wineryDataInputs.supportedLanguages?.length > 0
          ) {
          ...
          if (
            wineryDataInputs.amenities &&
            wineryDataInputs.amenities?.length > 0
          ) {
