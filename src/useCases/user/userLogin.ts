...
const userLogin = async (
  ...
  try {
    ...
    if (!user) {
      ...
      return {
        errors: inputErrors.concat(userResolversErrors.invalidCredentials),
      };
    } else {
      const userPassMatch = await argon2.verify(
        user.password,
      ...
      if (!userPassMatch) {
        ...
        const reservedServicesIds = await getConnection().query(
          SQL_QUERY_GET_RESERVED_SERVICES_IDS,
          [user.id]
        );

        if (reservedServicesIds[0].reservedServicesIds.length > 0 && user) {
          user.reservedServicesIds = reservedServicesIds[0]
            .reservedServicesIds as number[];
        }

        if (user.userType === UserType.WINERY_OWNER) {
          const createdWinery = await Winery.findOne({
          ...
          if (createdWinery && user) {
            ...
            return {
              errors: inputErrors.concat(
                userResolversErrors.wineryFromUserDeleted
              ),
            };
          }
        } else {
