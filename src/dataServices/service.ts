...
const experiences = async (realLimit: number) => {
  ...
  return await getConnection().query(
    SQL_QUERY_SELECT_PAGINATED_EXPERIENCES,
    replacements
  );
};
...
const findServiceNotMadeByCreatorByServiceAndCreatorId = async (
  serviceId: number,
  userId: number
) => {
...
const findServiceByParentIdAndStartDateTime = async (
  serviceId: number,
  startDateTime: Date
) => {
  return await Service.findOne({
...
const updateAttendeesByIdAndCreator = async (
  id: number,
  creatorId: number,
  noOfAttendees: number,
  noOfAttendeesService: number
) => {
