...
const findIdsFromServicesReservedByUserId = async (userId: number) => {
  return await getConnection().query(SQL_QUERY_GET_RESERVED_SERVICES_IDS, [
    userId,
  ]);
};

const findUserReservations = async (userId: number) => {
  const findAndCountResponse = await ServiceReservation.findAndCount({
    where: { userId: userId },
  });
...
const findWineryReservations = async (wineryId: number) => {
  ...
  return await ServiceReservation.find({
    where: { serviceCreatorId: winery?.creatorId },
  });
};
...
const findUserReservationByIdAndUserId = async (
  serviceId: number,
  userId: number
) => {
  return await ServiceReservation.findOne({ where: { serviceId, userId } });
};
...
const payPalReports = async (paypalId: string | null) => {
  ...
  if (paypalId) {
    qs.andWhere("experienceServices.paypalOrderId = :paypalOrderId ", {
