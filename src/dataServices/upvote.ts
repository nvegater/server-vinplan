...
import { SQL_QUERY_UPDATE_UPVOTE, 
    SQL_QUERY_UPDATE_POST_POINTS,
    SQL_QUERY_INSERT_NEW_UPVOTE } from "../resolvers/Universal/queries"

const findVoteByUserAndPost = async (postId: number, userId : number) => {
    return await Upvote.findOne({where: {postId, userId}});
}

const changeVote = async (postId: number, userId : number, realValue: number) => {
